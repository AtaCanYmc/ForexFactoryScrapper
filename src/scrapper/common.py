...
def to_24h(day, month, year, am_pm, last=None):
    """Convert an am/pm string to a datetime in 24-hour format with a timezone offset used by scrapers.

    The original scrapers added a fixed offset of 0 hours (add_h = 0). We preserve that behaviour
...
def parse_calendar_from_html(html, url):
    ...
    matchObj = re.search(r"([a-zA-Z]{3}) ([a-zA-Z]{3}) ([0-9]{1,2})", startDate.text)
    if not matchObj:
        logger.error("Failed to parse start date from text: %s", startDate.text)
        raise ValueError("Failed to parse start date")

    month = BASE_MONTH_NUMBERS.get(matchObj.group(2), None)
